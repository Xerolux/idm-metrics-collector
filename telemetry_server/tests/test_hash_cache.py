...
async def test_smart_caching(tmp_path):
    ...
    with patch("app.MODEL_DIR", str(tmp_path)):
        ...
        with patch("app.HASH_CACHE_TTL", 0.1):
            ...
            with patch(
                "app._get_file_hash_sync", side_effect=lambda f: "recalculated_hash"
            ) as mock_hash:
                ...
                assert mock_hash.call_count == 0, (
                    "Should not re-calculate hash if file unchanged"
                )
        ...
        with patch("app.HASH_CACHE_TTL", 0.1):
            with patch(
                "app._get_file_hash_sync", side_effect=lambda f: "new_hash_from_calc"
            ) as mock_hash_new:
