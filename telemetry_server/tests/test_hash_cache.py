...
from app import get_file_hash, _file_hash_cache, _get_file_hash_sync
...
async def test_smart_caching(tmp_path):
    ...
    # Ensure Model Directory checks don't fail path validation if needed
    # But get_file_hash calls _get_file_hash_sync which just opens the file.

    # Patch HASH_CACHE_TTL to a small value
    # Note: We must patch it where it is used (app.HASH_CACHE_TTL)
    with patch("app.HASH_CACHE_TTL", 0.1):
        ...
        # We make it return a DIFFERENT hash so we can distinguish re-calc vs cache.

        # IMPORTANT: Since run_in_executor pickles the function,
        # mocking _get_file_hash_sync might be tricky if not done at module level.
        # But app._get_file_hash_sync is a function.

        with patch("app._get_file_hash_sync", side_effect=lambda f: "recalculated_hash") as mock_hash:
             hash2 = await get_file_hash(filepath)

             # Before optimization:
             # TTL expired -> calls _get_file_hash_sync -> returns "recalculated_hash"

             # After optimization:
             # TTL expired -> checks stat -> unchanged -> returns cached real_hash

             # We assert the optimized behavior.
             # This assertion is expected to FAIL until we implement the fix.
