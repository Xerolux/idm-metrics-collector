# 🔥 IDM Metrics Collector - AI-Powered Heat Pump Monitoring & Control

<div align="center">

[![GitHub Release][releases-shield]][releases]
[![Downloads][downloads-shield]][releases]
[![License][license-shield]](LICENSE)
[![Discord][discord-shield]][discord]
[![Community Forum][forum-shield]][forum]
[![Docker Pulls](https://img.shields.io/docker/pulls/xerolux/idm-metrics-collector?style=for-the-badge)](https://hub.docker.com/r/xerolux/idm-metrics-collector)

</div>

## 🎯 Die intelligente All-in-One Monitoring & Steuerungslösung für IDM Wärmepumpen

> **Künstliche Intelligenz trifft auf Smart Home** - Die professionelle Monitoring-Plattform mit integrierter **KI-Anomalieerkennung**, **Home Assistant Integration** und **vollautomatischer Steuerung** für IDM Navigator 2.0 Wärmepumpen.

<div align="center">

### 🚀 Echtzeit-Überwachung • 🤖 Machine Learning • 📊 Langzeitanalyse • 🏠 Home Assistant • ⚡ Vollautomatisierung

</div>

### 🔒 Local First

Alle Messdaten, Dashboards und KI-Auswertungen bleiben in deiner eigenen Installation. Die lokale
KI läuft im mitgelieferten `ml-service`; es gibt keinen Telemetrie-Upload und keinen zentralen
Community- oder Modellserver. Ausgehende Integrationen wie MQTT, Benachrichtigungen, WebDAV und
Update-Prüfungen sind optional und standardmäßig deaktiviert.

---

**Keywords:** *Heat Pump Monitoring, IDM Wärmepumpe, AI Anomaly Detection, Machine Learning HVAC, Smart Heating Control, Home Automation, MQTT Integration, Modbus TCP, Energy Efficiency Monitor, COP Analysis, Predictive Maintenance, Smart Home Heat Pump, KI-gesteuerte Heizung, Intelligente Heizungssteuerung, Wärmepumpen-Überwachung, IoT Heating System*

<div align="center">

![Demo Animation](docs/images/demo.gif)

*Echtzeitüberwachung Ihrer IDM Wärmepumpe mit integrierter KI-Anomalieerkennung*

</div>

---

## 📸 Screenshots - Moderne Benutzeroberfläche

<details>
<summary><b>🖼️ Galerie anzeigen (12 Screenshots)</b></summary>
<br>

| 📊 Dashboard Übersicht | 🎮 Intuitive Steuerung |
|:---:|:---:|
| ![Hauptseite](docs/images/screenshots/Hauptseite.png) | ![Steuerung](docs/images/screenshots/Steuerung.png) |
| *Live-Daten aller Sensoren auf einen Blick* | *Direkte Kontrolle über Betriebsmodi und Sollwerte* |

| 📅 Intelligente Zeitpläne | 📝 Detaillierte Protokolle |
|:---:|:---:|
| ![Zeitplan](docs/images/screenshots/Zeitplan.png) | ![Protokoll](docs/images/screenshots/Protokoll.png) |
| *Wochenplan mit Drag & Drop Editor* | *Lückenlose Dokumentation aller Ereignisse* |

| ⚙️ Einfache Konfiguration | 📈 Grafana Integration |
|:---:|:---:|
| ![Einstellungen](docs/images/screenshots/Einstellungen.png) | ![Grafana](docs/images/screenshots/grafana_dashboard.png) |
| *Alle Settings an einem Ort* | *Professionelle Langzeitanalyse* |

| 🔔 Alarm Verwaltung | 📱 Responsive Design |
|:---:|:---:|
| ![Alarme](docs/images/screenshots/Alarme.png) | ![Mobile Ansicht](docs/images/screenshots/Mobile_View.png) |
| *KI-Warnungen und Systemalarme* | *Perfekt bedienbar auf jedem Gerät* |

</details>

---

## 📚 Vollständige Dokumentation im Wiki

**Für eine detaillierte Anleitung zur Installation, Konfiguration und Nutzung, besuchen Sie bitte unser [GitHub Wiki](https://github.com/Xerolux/idm-metrics-collector/wiki)!**

Dort finden Sie unter anderem:

*   [Installation & Setup](https://github.com/Xerolux/idm-metrics-collector/wiki/Installation-and-Setup)
*   [Konfiguration & Handbuch](https://github.com/Xerolux/idm-metrics-collector/wiki/Configuration)
*   [Architektur](https://github.com/Xerolux/idm-metrics-collector/wiki/Architecture)
*   [FAQ & Häufige Fragen](https://github.com/Xerolux/idm-metrics-collector/wiki/FAQ)

## 🌟 Projekt-Status & Roadmap
Siehe [ROADMAP.md](ROADMAP.md) für geplante Features und [CHANGELOG.md](CHANGELOG.md) für die neuesten Updates.

---

[releases-shield]: https://img.shields.io/github/v/release/Xerolux/idm-metrics-collector?style=for-the-badge
[releases]: https://github.com/Xerolux/idm-metrics-collector/releases
[downloads-shield]: https://img.shields.io/github/downloads/Xerolux/idm-metrics-collector/total?style=for-the-badge
[license-shield]: https://img.shields.io/github/license/Xerolux/idm-metrics-collector?style=for-the-badge
[discord-shield]: https://img.shields.io/discord/1234567890?color=5865F2&label=Discord&logo=discord&logoColor=white&style=for-the-badge
[discord]: https://discord.gg/your-discord-invite
[forum-shield]: https://img.shields.io/discourse/posts?color=333333&label=Forum&logo=discourse&logoColor=white&server=https%3A%2F%2Fcommunity.home-assistant.io&style=for-the-badge
[forum]: https://community.home-assistant.io/
