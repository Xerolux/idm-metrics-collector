...
class Luxtronik21Driver(HeatpumpDriver):
    ...
    PARAMETERS = {
        "temp_outside": {"index": 10, "type": "temperature", "category": SensorCategory.TEMPERATURE, "name": "Außentemperatur"},
        "temp_outside_avg": {"index": 11, "type": "temperature", "category": SensorCategory.TEMPERATURE, "name": "Außentemperatur Mittel"},
        "temp_hot_water": {"index": 17, "type": "temperature", "category": SensorCategory.TEMPERATURE, "name": "Warmwasser Ist"},
        "temp_hot_water_target": {"index": 18, "type": "temperature", "category": SensorCategory.CONTROL, "name": "Warmwasser Soll"},
        "temp_flow": {"index": 10, "type": "temperature", "category": SensorCategory.TEMPERATURE, "name": "Vorlauf"}, # Check ID
        "temp_return": {"index": 11, "type": "temperature", "category": SensorCategory.TEMPERATURE, "name": "Rücklauf"}, # Check ID
        "temp_source_in": {"index": 19, "type": "temperature", "category": SensorCategory.TEMPERATURE, "name": "Wärmequelle Ein"},
        "temp_source_out": {"index": 20, "type": "temperature", "category": SensorCategory.TEMPERATURE, "name": "Wärmequelle Aus"},
        "compressor_output": {"index": 257, "type": "percent", "category": SensorCategory.STATUS, "name": "Verdichter Leistung"},
        "heat_quantity_heating": {"index": 151, "type": "energy", "category": SensorCategory.ENERGY, "name": "Wärmemenge Heizen"},
        "heat_quantity_hot_water": {"index": 152, "type": "energy", "category": SensorCategory.ENERGY, "name": "Wärmemenge WW"},
    }
    ...
    def get_sensors(self, config: Dict[str, Any]) -> List[SensorDefinition]:
        ...
        for sensor_id, info in self.PARAMETERS.items():
            sensors.append(SensorDefinition(
                ...
                unit="°C" if info["type"] == "temperature" else ("kWh" if info["type"] == "energy" else ""),
                address=info["index"],
            ...
            "charts": [
                {
                    ...
                    "queries": [
                        ...
                        {"label": "Quelle Ein", "query": "temp_source_in", "color": "#10b981"},
                        {"label": "Quelle Aus", "query": "temp_source_out", "color": "#f59e0b"}
                    ],
                    "hours": 24
                }
            ]
