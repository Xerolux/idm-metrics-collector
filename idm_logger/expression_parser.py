...
class ExpressionParser:
    ...
    def _evaluate_with_values(self, expression: str, values: Dict[str, float]) -> float:
        ...
        """
        # Functions and variables are handled directly by SafeExpressionEvaluator
    ...
    def evaluate_expression_series(self, expression: str) -> List[tuple]:
        ...
        # O(1) Precompile AST outside the loop
        try:
        ...
        # O(1) Prepare fast lookups for time series data
        time_series_dict = {
            label: {ts: val for ts, val in self.query_results.get(label, [])}
        ...
        for timestamp in sorted(all_timestamps):
            # Fast check if all required labels exist at this timestamp
            query_values = {}
            missing_data = False
            for label in unique_labels:
                if timestamp not in time_series_dict[label]:
                    missing_data = True
                    break
                query_values[label] = time_series_dict[label][timestamp]
