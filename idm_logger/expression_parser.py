...
class ExpressionParser:
    ...
    def evaluate_expression_series(self, expression: str) -> List[tuple]:
        ...
        # Find which queries are used
        query_labels = self.parse_expression(expr)

        # Convert time-series data to dicts for O(1) lookups
        query_data_dicts = {}
        for label in query_labels:
            if label not in self.query_results:
                return []
            query_data_dicts[label] = dict(self.query_results[label])
        ...
        for timestamp in sorted(all_timestamps):
            ...
            valid = True
            for label in query_labels:
                if timestamp not in query_data_dicts[label]:
                    valid = False
                    break
                values[label] = query_data_dicts[label][timestamp]
