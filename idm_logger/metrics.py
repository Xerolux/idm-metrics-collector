...
class MetricsWriter:
    ...
    def _send_data(self, data: Union[Dict, List[Dict]]) -> bool:
        ...
        tags = self._get_tags()
        # ⚡ Bolt: Pre-calculate the prefix outside the loop to avoid redundant string allocations
        ...
        for measurements in items:
            ...
            for key, value in measurements.items():
                if key.endswith("_str"):
                    continue
                # ⚡ Bolt: Use inline conditional instead of int() and use elif to avoid redundant checks
