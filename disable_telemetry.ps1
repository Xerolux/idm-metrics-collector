...
Write-Output "Disabling telemetry and restarting idm-logger..."
...
# Recreate the container with TELEMETRY_ENABLED=false
Write-Output "Creating new container with TELEMETRY_ENABLED=false..."
$envVars = "-e IDM_HOST=192.168.178.103 -e IDM_PORT=502 -e METRICS_URL=http://victoriametrics:8428/write -e INTERNAL_API_KEY=change_me_secure_key -e TELEMETRY_SHARED_TOKEN=COMMUNITY-CONTRIBUTOR-TOKEN-2026 -e TELEMETRY_ENABLED=false"
...
& $plinkPath -ssh root@$server -pw $password -batch -hostkey $hostKey "docker logs idm-logger --tail 30"

Write-Output "Update complete! Telemetry is now disabled."