...
## 2026-08-21 - Explicit Form Control Labels inside Cards
**Learning:** When using generic layout components (like PrimeVue Cards) to wrap form controls, relying on visual proximity or generic  slots is insufficient for accessibility, as screen readers cannot inherently associate the card's title with the input inside its content slot.
**Action:** Always convert card titles to `<label>` elements and explicitly bind them to their respective inputs using dynamically generated `id` / `inputId` and `for` attributes (e.g. `:for="'input-' + index"`).
