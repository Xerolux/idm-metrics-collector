...
## 2026-05-08 - Added aria-label to missing search input and explicit labels to ExportDialog inputs
**Learning:** Using aria-label for non-label search inputs and adding for attributes to existing labels is necessary to provide clear, accessible component interfaces.
**Action:** Add id attributes to all inputs linked to labels via for attributes, and add explicit aria-labels on bare input fields missing explicit contextual labels.
