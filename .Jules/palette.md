...
## 2026-07-29 - Decorative Icons Accessibility
**Learning:** In Vue, PrimeIcons (like `<i class="pi pi-exclamation-circle">`) used decoratively next to error messages or success text are announced redundantly by screen readers if not hidden, especially when wrapped in a `role="alert"` container.
**Action:** Always add `aria-hidden="true"` to decorative `<i>` elements and ensure dynamic error messages use `role="alert"` for proper announcements.
