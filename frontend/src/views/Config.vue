      ...
      >
        <div class="p-6 overflow-y-auto flex-grow">
          ...
          <div v-if="activeCategory === 'connection'" class="flex flex-col gap-6">
            ...
            <Fieldset legend="Lokale KI & Datenschutz" :toggleable="true">
              <div class="flex flex-col gap-4">
                <div class="bg-green-900/20 border border-green-600/50 p-4 rounded flex gap-3">
                  <i class="pi pi-lock text-green-400 text-xl mt-1"></i>
                  <div>
                    <strong class="text-green-300">100 % lokale Verarbeitung</strong>
                    <p class="text-sm text-gray-200 mt-1">
                      Messdaten und KI-Modelle bleiben in deiner Installation. Es werden keine
                      Telemetriedaten, Zugangsdaten oder Modelle an einen Community-Server gesendet.
                    </p>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <Checkbox v-model="config.ai.enabled" binary inputId="local_ai_enabled" />
                  <label for="local_ai_enabled" class="font-bold cursor-pointer">
                    Lokale KI-Anomalieerkennung aktivieren
                  </label>
                </div>
                <small class="text-gray-300">
                  Training und Auswertung laufen ausschließlich über den lokalen ML-Dienst.
                </small>
              </div>
            </Fieldset>
          </div>
                ...
                <div class="bg-gray-800 p-4 rounded border border-gray-700 mt-4">
                  <div class="flex flex-col gap-4">
                    <div class="flex flex-col gap-2">
                      ...
                      <div class="rounded border border-gray-700 bg-gray-900 p-3 text-sm">
                        Lokaler Autoencoder mit kontinuierlichem Lernen
                      </div>
                      <small class="text-gray-400">
                        Das Modell lernt ausschließlich aus den Messdaten dieser Installation.
                      </small>
                    </div>
                    <div class="flex flex-col gap-2">
...
const copyId = async () => {
  ...
  toast.add({
    ...
    summary: success ? 'Kopiert' : 'Fehler',
    detail: success ? 'ID in Zwischenablage kopiert' : 'Konnte nicht kopiert werden',
    life: 3000
  })
}
...
onMounted(async () => {
  try {
    ...
    loadAiStatus()
    // Refresh the local AI status periodically.
