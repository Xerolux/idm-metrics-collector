<template>
  <div class="p-4 flex flex-col gap-4">
              ...
              <div v-if="config.ai.enabled" class="flex flex-col gap-6">
                ...
                <div class="bg-gray-800 p-4 rounded border border-gray-700 mt-4">
                  ...
                  <div class="mt-4 text-xs text-gray-500 text-center">
                    Hinweis: Alarme können über Grafana konfiguriert werden (Metrik:
                    <code>idm_anomaly_flag</code>).
                  </div>
                </div>
              </div>
            </Fieldset>
            ...
            <div class="bg-gray-800 rounded-lg p-4 border border-gray-700 flex flex-col gap-3">
              ...
              <div class="flex flex-col gap-2 mt-2 border-t border-gray-700 pt-2">
                ...
                <div class="flex gap-2">
                  <SelectButton
                    v-model="config.updates.channel"
                    :options="['latest', 'beta', 'release']"
                    :allowEmpty="false"
                    class="w-full"
                  />
                </div>
    ...
    <div
      class="flex gap-4 mt-4 justify-end border-t border-gray-700 pt-4 sticky bottom-0 bg-gray-900/90 backdrop-blur p-4 z-10"
    >
      <Button
        label="Speichern"
        icon="pi pi-save"
  ...
  try {
    const [updateRes, signalRes] = await Promise.all([
      axios.get('/api/check-update', {
        params: { channel: config.value.updates?.channel || 'latest' }
      }),
...
const checkUpdates = async () => {
  ...
  try {
    const res = await axios.get('/api/check-update', {
      params: { channel: config.value.updates.channel }
    })
      ...
      try {
        ...
        if (res.data.success) {
          toast.add({
            ...
            detail: 'System wird aktualisiert...',
            life: 5000
          })
        ...
        toast.add({
          severity: 'error',
          summary: 'Fehler',
          detail: e.response?.data?.error || 'Update fehlgeschlagen',
          life: 5000
        })
      } finally {
