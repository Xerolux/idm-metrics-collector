...
class TestMLServiceMulti(unittest.TestCase):
    ...
    def test_multi_heatpump_processing(self):
        ...
        with patch.object(ml_main, "create_pipeline", return_value=mock_pipeline_instance):
            with patch.object(ml_main, "requests") as mock_requests:
                ...
                mock_response = {
                    ...
                    "data": {
                        "result": [
                            {
                                "metric": {"__name__": "idm_heatpump_temp_outdoor", "heatpump_id": "hp1"},
                                "value": [1234567890, "10.5"]
                            },
                            {
                                "metric": {"__name__": "idm_heatpump_temp_outdoor", "heatpump_id": "hp2"},
                                "value": [1234567890, "12.0"]
                            },
                            {
                                "metric": {"__name__": "idm_heatpump_power_current", "heatpump_id": "hp1"},
                ...
                for p in payloads:
                    if 'heatpump_id="hp1"' in p:
                        hp1_found = True
                self.assertTrue(hp1_found, f"Did not find payload for hp1 in {payloads}")

                hp2_found = False
                for p in payloads:
                    if 'heatpump_id="hp2"' in p:
                        hp2_found = True
                self.assertTrue(hp2_found, f"Did not find payload for hp2 in {payloads}")
        ...
        with patch.object(ml_main, "requests") as mock_requests:
            mock_response = {
                ...
                "data": {
                    "result": [
                        {
                            "metric": {"__name__": "idm_heatpump_sensor1", "heatpump_id": "hp1"},
                            "value": [0, "100"]
                        },
                        {
                            "metric": {"__name__": "idm_heatpump_sensor1", "heatpump_id": "hp2"},
                            "value": [0, "200"]
                        },
                        {
                            "metric": {"__name__": "idm_heatpump_sensor2", "heatpump_id": "hp1"},
                            "value": [0, "300"]
                        }
                    ]
                }
