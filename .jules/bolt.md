...
## 2026-05-13 - O(1) AST mathematical expression evaluation
**Learning:** Parsing mathematical expressions to AST and resolving dynamically replacing variable representations internally using AST instead of string interpolation allows evaluation latency to reduce by magnitudes. Additionally using dictionary-of-dictionaries lookups yields O(1) performance compared to using tuple arrays which scales as O(N) when timestamp filtering is employed.
**Action:** When performing expression evaluation over time-series data, precompile AST structures ahead of time, iterate natively via safe overridden ast.NodeVisitors, and rely on dict mappings to achieve optimal O(1) performance instead of iterating over sets of tuples and evaluating regex replacements on strings for every tick.
