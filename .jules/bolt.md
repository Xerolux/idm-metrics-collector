...
## 2026-07-01 - Prometheus Metrics Collection Overhead
**Learning:** When retrieving Prometheus metrics in Python applications (like `admin_get_metrics` in `telemetry_server/app.py`), calling `collector.collect()` inside a loop over metric requests causes severe O(N^2) performance degradation because it evaluates the entire registry for every single metric fetched.
**Action:** Perform a single metric collection pass and cache the results keyed by `metric.name` (not `sample.name`) at the start of the endpoint, enabling O(1) dictionary lookups for subsequent individual metric retrieval while preserving base metric name semantics.
