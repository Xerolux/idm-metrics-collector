...
## 2026-05-15 - new Function(...) compilation inside hot loops
**Learning:** Using naive string substitution and creating a `new Function(...)` repeatedly inside a time-series evaluation loop (like in `evaluateExpressionSeries` in the Vue frontend) blocks the main thread and scales O(N) with the dataset size.
**Action:** Precompile the function once outside the loop using `new Function(...)`, injecting required utility functions (like `sum`, `avg`, `min`, `max`) as arguments. Use a time-based O(1) Map for variable lookups, then iterate and call the precompiled function.
