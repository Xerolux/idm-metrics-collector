...
## 2026-05-18 - Evaluate ASTs in O(1) for Time-series Expressions
**Learning:** Re-evaluating expressions dynamically using regex substitutions (`re.sub`) or re-parsing an AST string inside a loop (e.g. over time-series data) adds tremendous overhead.
**Action:** When evaluating mathematical expressions over time-series data, achieve O(1) performance by precompiling the AST using `ast.parse` outside the evaluation loop, structuring time-series data into dictionaries for O(1) lookups, and injecting variables through an overridden `visit_Name` in an `ast.NodeVisitor` rather than using string substitutions.
