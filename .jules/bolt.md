...
## 2026-04-10 - O(1) AST Evaluation for Time Series
**Learning:** Evaluating math expressions (like `(A+B)/2`) over timeseries data inside a hot loop is very slow (O(N*M)) if `ast.parse` and regex replacements `re.sub` are performed for every single timestamp.
**Action:** When optimizing mathematical expression evaluation over time-series data (e.g., `evaluate_expression_series` in `idm_logger/expression_parser.py`), achieve O(1) loop performance by precompiling the AST using `ast.parse` *outside* the evaluation loop, structuring time-series list data into dictionaries for O(1) lookups, and injecting variables through an overridden `visit_Name` in an `ast.NodeVisitor` rather than using regex string substitutions.
