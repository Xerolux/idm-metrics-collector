...
## 2026-04-26 - MutableRow wrapper overhead
**Learning:** Custom wrappers around `sqlite3.Row` (like `MutableRow`) to allow mutation add significant attribute access overhead in tight loops.
**Action:** When mutable rows are needed after fetching, cast directly to a native dictionary via `dict(row)` instead of wrapping.
