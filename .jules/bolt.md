...
## 2026-05-15 - Inline Conditionals vs Function Calls in Hot Loops
**Learning:** In tight Python loops formatting thousands of data points (like Line Protocol metrics), replacing an `int(value)` function call with an inline conditional (`1 if value else 0`) for boolean conversion significantly reduces execution time by avoiding local/global name lookups and function call overhead.
**Action:** When converting booleans to integers inside hot loops, always prefer fast inline conditionals over `int()` casting.
