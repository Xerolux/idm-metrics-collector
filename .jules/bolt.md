...
## 2025-02-12 - O(N^2) Prometheus Metric Collection

**Learning:** When retrieving Prometheus metrics in Python applications (like `telemetry_server/app.py`), calling `collector.collect()` inside a loop over metric names leads to severe O(N^2) performance degradation because `collect()` can be quite slow and return all metrics each time. Also, checking for exact match on `metric.name` requires storing the full sample list for lookup.
**Action:** Perform a single collection pass (`collector.collect()`), cache the results keyed by `metric.name` (the base name, e.g., `telemetry_requests`), and perform O(1) lookups in `get_metric_value(metric_name, labels)`.
