...
## 2024-05-24 - Line Protocol String Allocation in Hot Loops
**Learning:** In `idm_logger/metrics.py`, evaluating stringified keys with prefixes (like `idm_heatpump{tags} `) inside a metrics parsing loop incurs a high performance cost due to redundant f-string allocations. Also, because `bool` is a subclass of `int` in Python, sequential `if isinstance()` checks for `bool` and `(int, float)` result in redundant execution unless structured with `elif`.
**Action:** Pre-calculate common string prefixes outside formatting loops. Use inline conditionals (`1 if value else 0`) over `int()` for speed, and use `elif` to prevent redundant type checking to maximize inner-loop speed.
