...
## 2025-01-24 - Pre-compile dynamic JS functions for time-series evaluation
**Learning:** Using `new Function(...)` and string regex substitutions repeatedly inside a tight loop over thousands of time-series points causes severe main thread blocking and massive performance degradation (over 1000ms overhead for 10k points).
**Action:** Pre-compile the JS function outside the loop and use `Map` structures for O(1) time-based variable lookups. This drops execution time from 1.4s to ~40ms on large datasets.
