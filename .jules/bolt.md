...
## 2026-05-25 - REGISTRY.collect() O(N^2) Overhead
**Learning:** Calling `REGISTRY.collect()` or `collector.collect()` inside a loop for multiple metric lookups causes severe performance degradation due to repeated system metric evaluation, resulting in O(N*M) runtime complexity.
**Action:** When querying Prometheus metrics locally, optimize by performing a single collection pass and caching the results into a dictionary mapping `metric.name` to `metric.samples` for O(1) lookups instead of O(N) list iteration. Ensure matching logic handles sample name suffixes like `_total` or `_seconds`.
