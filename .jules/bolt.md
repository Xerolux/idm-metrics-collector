...
## 2026-07-17 - Pre-compiling Functions in Hot Loops
**Learning:** In the frontend, when evaluating mathematical expressions over large datasets iteratively, using string regex substitutions and `new Function(...)` inside the loop blocks the main thread and severely impacts performance.
**Action:** Precompile the JS function once outside the loop and use an O(1) time-based Map for variable lookups instead of rebuilding and parsing the expression on every iteration.
