...
## 2026-04-10 - Replace SQLite Row Wrapper with Native Dict
**Learning:** Custom wrappers around `sqlite3.Row` (like `MutableRow`) for mutability add significant Python overhead on property access inside hot loops (upwards of 3x slower).
**Action:** Always convert directly to a standard `dict` via `[dict(row) for row in rows]` when mutability is required, avoiding complex wrapper classes.
