...
## 2024-05-19 - Fast Prometheus Metric Extraction
**Learning:** In `telemetry_server/app.py`, calling `REGISTRY.collect()` or `collector.collect()` inside a loop for multiple metric lookups causes severe performance degradation due to repeated system metric evaluation.
**Action:** Optimize by performing a single collection pass and caching the `(metric.name, sample.labels, sample.value)` tuples into a list to iterate over.
