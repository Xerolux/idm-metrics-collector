...
## 2026-04-10 - Optimize AST Expression Evaluation
**Learning:** Evaluating mathematical expressions over time-series data using `ast.parse` inside a loop, combined with regex-based string substitutions for variables, creates significant aggregate overhead.
**Action:** Achieve O(1) performance by precompiling the AST using `ast.parse` outside the evaluation loop, structuring time-series data into dictionaries for O(1) lookups, and injecting variables through an overridden `visit_Name` in an `ast.NodeVisitor` rather than using regex string substitutions.
