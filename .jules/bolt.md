...
## 2026-05-16 - Prometheus REGISTRY.collect() in O(1) Time
**Learning:** In the `admin_get_metrics` endpoint, iterating over `REGISTRY.collect()` for each individual requested metric parses the entire registry multiple times causing O(N*M) time complexity.
**Action:** When extracting multiple metrics from the prometheus registry sequentially, parse `REGISTRY.collect()` a single time caching the metric names and label tuples to a list to iterate over, dropping the performance cost significantly by avoiding multiple system metric collections.
