...
## 2024-05-25 - Custom Dict Wrappers vs Native Dicts
**Learning:** Implementing custom wrapper classes (like `MutableRow`) over `sqlite3.Row` to support mutability introduces significant overhead during property access in fast loops.
**Action:** Use native built-in `dict(row)` casting instead of custom wrappers when mutability is required. Native dicts are written in C, vastly improving instantiation and read/write speeds while reducing overall memory overhead.
