...
## 2026-04-10 - Optimizing Metrics Formatting
**Learning:** Using `isinstance()` and string concatenation (`f"idm_heatpump{tags} {field_str}"`) in hot loops for line protocol formatting incurs measurable overhead. While type checking with `type()` is slightly faster, `isinstance()` is kept for correctness with subclasses (like `IntEnum`). Stringifying only when appending and avoiding intermediate object allocations (like integer casting for booleans) speeds up loops.
**Action:** When formatting string representations of telemetry batches, always pre-calculate the common prefix (`idm_heatpump{tags} `) outside the item loop, kept `isinstance` for robustness against subclasses while converting boolean and avoiding redundant conversions, and memoize stringified keys to avoid redundant string allocations per record.
