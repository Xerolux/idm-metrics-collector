...
## 2024-06-11 - Optimize Prometheus Collection
**Learning:** Calling `collector.collect()` inside loops (like searching for metrics or iterating configurations) leads to an O(N^2) degradation when using Prometheus.
**Action:** Do a single collection pass upfront to construct a dictionary keyed by `metric.name`, enabling O(1) lookups during repeated metric retrieval.
