...
## 2026-04-10 - Optimize Line Protocol Metric Formatting
**Learning:** In hot loops (like formatting large batches of telemetry data), standard `int(boolean)` casting and consecutive `if` checking incur small but measurable CPU overhead. In combination with redundant string concatenation inside the loop, this causes unnecessary allocation and execution time penalties.
**Action:** Always pre-calculate common string prefixes outside the loop and append to them. Use `elif` for consecutive, mutually-exclusive type checks, and prefer inline conditionals (e.g. `1 if value else 0`) over `int(value)` casts for booleans to squeeze out extra inner-loop performance.
