...
## 2026-07-02 - O(N²) Prometheus Metric Collection
**Learning:** Calling `REGISTRY._collector_to_names.keys()` and `collector.collect()` inside a loop (e.g. iterating for each target metric to find its value) results in severe O(N²) performance degradation, as the entire metrics registry is fully collected multiple times.
**Action:** Perform a single collection pass outside the loop and cache the metric objects keyed by `metric.name` for O(1) lookups.
