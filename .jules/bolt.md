...
## 2026-04-10 - O(1) Prometheus Metric Caching
**Learning:** In `telemetry_server/app.py`, calling `REGISTRY.collect()` or `collector.collect()` inside a loop for multiple metric lookups causes severe performance degradation due to repeated system metric evaluation. Furthermore, searching by `metric.name` fails to handle Prometheus sample suffixes like `_total` (e.g., `telemetry_requests_total` is only found in `sample.name`, not `metric.name`).
**Action:** Optimize by performing a single collection pass and caching the results into a dictionary mapping `sample.name` to the samples for O(1) lookups instead of O(N*M) loop iteration. This prevents accidentally matching the base `metric.name` without handling metric suffixes like `_total`.
