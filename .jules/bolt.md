...
## 2026-05-15 - AST Parsing Overhead in Evaluation Loops
**Learning:** Evaluating mathematical expressions over time-series data using `ast.parse` inside the loop per timestamp causes significant parsing overhead. Furthermore, injecting variables via regex `re.sub` string substitutions incurs redundant parsing allocations and risks ReDoS.
**Action:** When evaluating AST expressions inside hot loops, precompile the AST using `ast.parse` once outside the evaluation loop. For variable injection, structure data into dictionaries for O(1) lookups and inject variables through an overridden `visit_Name` in an `ast.NodeVisitor` instead of string substitution.
