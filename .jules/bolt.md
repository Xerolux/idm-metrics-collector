...
## 2026-04-10 - O(N^2) Overhead in Prometheus Metric Retrieval
**Learning:** When retrieving Prometheus metrics dynamically, calling `collector.collect()` inside a loop checking multiple metric names scales quadratically, becoming extremely slow under load.
**Action:** Perform a single collection pass upfront and cache the results keyed by `metric.name` for O(1) lookups.
