...
## 2024-05-30 - Optimize Prometheus metric collection
**Learning:** `REGISTRY.collect()` or `collector.collect()` inside a loop for multiple metric lookups causes severe performance degradation due to repeated system metric evaluation.
**Action:** Optimize by performing a single collection pass and caching the results into a dictionary mapping `metric.name` to `metric.samples` for O(1) lookups instead of O(N) list iteration.
