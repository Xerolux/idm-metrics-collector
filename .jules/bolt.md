...
## 2026-05-18 - Admin Metrics Loop Optimization
**Learning:** Calling `collector.collect()` inside a loop for every Prometheus metric we want to read causes severe O(N^2) performance degradation, as the registry does a full collection pass every single time.
**Action:** When gathering multiple metrics at once, always do a single pass over `REGISTRY.collect()` and cache the metrics by name for O(1) lookups.
