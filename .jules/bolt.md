...
## 2024-05-24 - Avoid redundant Prometheus metric collection
**Learning:** When retrieving Prometheus metrics in Python applications, avoid calling `REGISTRY.collect()` or `collector.collect()` inside a loop, as it causes severe O(N^2) performance degradation.
