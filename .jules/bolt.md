...
## 2026-06-19 - Prometheus Metrics Collection Performance
**Learning:** Calling `collector.collect()` inside a loop while fetching specific metrics in `telemetry_server/app.py` results in O(N^2) performance degradation, taking hundreds of milliseconds for a few metric reads as every call sweeps all collectors and metrics entirely.
**Action:** When extracting multiple metrics, perform a single collection pass (`collector.collect()`) beforehand, cache the results in a dictionary mapped by `metric.name`, and execute O(1) cache lookups inside the getter function.
