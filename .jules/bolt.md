...
## 2026-06-03 - AST Parsing in Hot Loops
**Learning:** Re-evaluating mathematical expressions over large time-series data using regex string replacements inside a loop causes immense CPU overhead. By pre-parsing the expression into an AST outside the evaluation loop, we can reduce redundant string parsing costs and achieve O(1) performance using AST dictionary lookups and an `ast.NodeVisitor`.
**Action:** Pre-parse strings that remain constant across loops and optimize lookups, such as precompiling AST nodes for expressions evaluated over many points.
