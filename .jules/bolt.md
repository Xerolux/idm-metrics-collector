...
## 2026-05-25 - Prometheus collect() loop overhead
**Learning:** Calling `REGISTRY.collect()` or `collector.collect()` inside a loop for multiple metric lookups in a single request causes severe performance degradation as it re-evaluates all metrics.
**Action:** Perform a single `collector.collect()` pass and cache the results keyed by `metric.name` outside the lookup function to enable O(1) reads while preserving semantics.
