...
## 2026-07-04 - Prometheus Metric Collection O(N^2) Bottleneck
**Learning:** When retrieving Prometheus metrics in Python applications, avoid calling `REGISTRY.collect()` or `collector.collect()` inside a loop. It causes severe O(N^2) performance degradation because `collect()` evaluates all metrics for the collector.
**Action:** Perform a single collection pass and cache the results keyed by `metric.name` for O(1) lookups while preserving base metric name semantics.
