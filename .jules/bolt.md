...
## 2026-06-13 - Optimize Prometheus Metric Collection
**Learning:** Calling `collector.collect()` inside a loop (like for every metric searched) causes severe O(N^2) performance degradation because it performs an expensive re-collection for every iteration.
**Action:** When gathering multiple specific Prometheus metrics dynamically, perform a single `collector.collect()` pass upfront, storing results in a local dictionary cache keyed by `metric.name` for O(1) lookups.
