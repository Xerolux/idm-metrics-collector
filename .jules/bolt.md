...
## 2026-06-05 - Optimize Prometheus Metric Collection in Loop
**Learning:** Calling `REGISTRY.collect()` or `collector.collect()` inside a loop for metric lookups causes severe performance degradation, making the lookup O(N*M). Caching the results of a single collection pass by `metric.name` provides an O(1) lookup while preserving functional correctness.
**Action:** Pre-compute metric collections outside of loops and cache results by `metric.name` to preserve O(1) lookups and avoid calling `.collect()` repeatedly.
