...
## 2024-05-23 - AST parsing in ExpressionParser
**Learning:** Compiling AST with ast.parse inside the evaluation loop for each time-series timestamp in idm_logger/expression_parser.py was an O(N) performance bottleneck. Additionally, regex substitution per query label per timestamp caused major overhead. Furthermore, parse_expression() can return duplicate labels (e.g., 'A+A' -> ['A', 'A']), requiring a set() conversion when verifying label counts against dictionaries.
**Action:** Precompile the AST once outside the loop. Use an overridden visit_Name method in ast.NodeVisitor to resolve variables dynamically via O(1) dictionary lookups at execution time, avoiding regex string replacements and loop compilation.
