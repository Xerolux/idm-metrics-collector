...
## 2026-04-10 - O(1) AST Expression Evaluation over Time-Series
**Learning:** Repeatedly parsing mathematical expression ASTs and using regex to substitute variables in hot loops is extremely slow. Evaluating static string thresholds into floats on every single tick multiplies small parsing overheads. Structuring time-series data into dictionaries for O(1) lookups avoids inner O(T) search loops.
**Action:** Precompile the AST once using `ast.parse` outside the evaluation loop, structure time-series data into dictionaries for O(1) lookups, and inject variables via an overridden `visit_Name` in `ast.NodeVisitor` instead of string substitution.
