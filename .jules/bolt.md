...
## 2026-05-15 - Prometheus Metrics Collection Overhead
**Learning:** Calling `REGISTRY.collect()` or `collector.collect()` inside a loop for multiple metric lookups (e.g. `admin_get_metrics`) causes severe performance degradation due to repeated system metric evaluation, resulting in O(N) evaluations where N is the number of metrics.
**Action:** Optimize by performing a single collection pass and caching the results into a dictionary mapping `metric.name` to `metric.samples` for O(1) lookups instead of O(N) evaluation.
