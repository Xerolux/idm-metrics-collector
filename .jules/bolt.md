...
## 2025-03-01 - Avoid duplicate string allocations in loops
**Learning:** In `idm_logger/metrics.py`, evaluating static formatting (like `f"idm_heatpump{tags} "`) inside an iteration over dictionaries caused redundant string allocations on every line protocol append. Furthermore, reassigning variables before type-checking (like `int(value)` on booleans) caused redundant type matching checks `isinstance(value, (int, float))` when standard `elif` structure could suffice.
**Action:** When creating line protocols or formatted strings in loops, always hoist invariant parts to variables outside the loop. Use `elif` conditionals and inline ternaries to avoid unnecessary redundant passes or variable re-assignments in hot paths.
