...
## 2026-04-10 - O(1) Lookups and AST Precompilation for Fast Expression Evaluation
**Learning:** When evaluating string-based mathematical expressions dynamically over large time-series data using string concatenation and reparsing the AST per loop iteration, performance degradation is massive. String concatenation creates large amounts of throw-away garbage, and parsing an AST from a string is extremely expensive.
**Action:** Achieve massive performance gains in hot evaluation loops by precompiling the mathematical expression into an AST object `ast.parse` once outside the loop. Use an overridden `ast.NodeVisitor` (`visit_Name`) to look up and inject variable values contextually from a precomputed O(1) dictionary structure instead of continuously string-substituting variable names and calling `eval()` or `ast.parse()` per timestamp.
