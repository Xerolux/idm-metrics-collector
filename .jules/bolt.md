...
## 2026-05-18 - Repeated System Metric Collection
**Learning:** Calling `REGISTRY.collect()` inside a loop for multiple metric lookups causes severe performance degradation because it triggers the evaluation of system metrics repeatedly.
**Action:** Perform a single collection pass and cache the `(metric.name, sample.labels, sample.value)` tuples into a list, then iterate over this list to lookup values, significantly reducing execution time.
