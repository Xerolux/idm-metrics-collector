...
## 2026-05-15 - Optimize SQLite Row Mutation
**Learning:** Using a custom wrapper class (like `MutableRow`) around `sqlite3.Row` to allow mutation introduces significant attribute access overhead in hot loops (e.g., iterating through scheduled jobs).
**Action:** When mutability is required for `sqlite3.Row` objects fetched from the database, avoid using custom Python wrapper classes. Instead, convert the row directly to a native dictionary using `dict(row)`.
