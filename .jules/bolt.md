...
## 2026-05-18 - Caching Prometheus Metric Collection
**Learning:** Calling `REGISTRY.collect()` inside a loop for metric lookups (like in `get_metric_value`) causes severe performance degradation, running an O(N) operation repeatedly. Pre-collecting metrics is much faster, but it is critical to cache them by the base `metric.name` rather than the `sample.name` (which includes suffixes like `_total`) to maintain lookup semantics and avoid functional regressions for existing queries.
**Action:** Always perform a single collection pass and cache the results before doing multiple metric lookups in a function. Ensure the cache is keyed by the correct identifier expected by callers.
