...
## 2026-04-10 - Custom MutableRow Wrapper Overhead
**Learning:** Custom wrapper classes around native objects (like `MutableRow` for `sqlite3.Row`) introduce significant Python-level method overhead for lookups and iterations, making them much slower than native C-implemented types.
**Action:** When mutability is needed for database rows fetched with `fetchall()`, convert them directly to native dictionaries using `dict(row)` instead of creating custom wrapper objects.
