...
## 2026-05-21 - Optimize Prometheus Metric Collection
**Learning:** Calling `REGISTRY.collect()` inside a loop for multiple specific metric lookups causes redundant, expensive system evaluation of *all* metrics each time.
**Action:** When extracting multiple specific metrics, do a single `collector.collect()` pass over the registry upfront, caching the results into a fast O(1) dictionary mapping `metric.name` to `metric.samples` before looking them up.
