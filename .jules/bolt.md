...
**Learning:** Calling `REGISTRY.collect()` or `collector.collect()` inside a loop for metric lookups causes severe performance degradation, because each call re-evaluates all metrics.
**Action:** When performing multiple metric lookups in a single request (e.g. for an admin metrics dashboard), perform a single collection pass at the beginning, cache the results keyed by `metric.name`, and perform O(1) lookups on the cache.
